...
export default function EditPriceTab({ vehicle, onSave }: EditPriceTabProps) {
  ...
  const handleSave = async () => {
    ...
    if (!formData.ubicacion || !formData.coordinates) {
      Alert.alert(
        "Ubicación Requerida",
        "Por favor selecciona la ubicación del vehículo"
      );
      return;
    }
    ...
    if (formData.mileageLimit === "limited" && (!dailyKm || dailyKm < 50)) {
      Alert.alert(
        "Error",
        "Si limitas el kilometraje, debe ser al menos 50 km/día"
      );
      return;
    }
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.discountCard}>
          ...
          <View style={styles.discountInputContainer}>
            <TextInput
              ...
              onChangeText={(text) =>
                setFormData({ ...formData, weeklyDiscount: text })
              }
        ...
        <View style={styles.discountCard}>
          ...
          <View style={styles.discountInputContainer}>
            <TextInput
              ...
              onChangeText={(text) =>
                setFormData({ ...formData, monthlyDiscount: text })
              }
              keyboardType="number-pad"
              placeholder="20"
              maxLength={2}
                ...
                <View style={styles.previewPriceContainer}>
                  ...
                  <Text style={styles.previewDiscounted}>
                    ...
                    {(
                      ...
                      7 *
                      (1 - (parseInt(formData.weeklyDiscount) || 0) / 100)
                    ).toFixed(2)}
                  </Text>
                </View>
              </View>
            )}
            {parseInt(formData.monthlyDiscount) > 0 && formData.precio && (
              <View style={styles.previewRow}>
                <Text style={styles.previewLabel}>30 días:</Text>
                <View style={styles.previewPriceContainer}>
                  <Text style={styles.previewOriginal}>
                    ${((parseFloat(formData.precio) || 0) * 30).toFixed(2)}
                  </Text>
                  <Text style={styles.previewDiscounted}>
                    $
                    {(
                      (parseFloat(formData.precio) || 0) *
                      30 *
                      (1 - (parseInt(formData.monthlyDiscount) || 0) / 100)
                    ).toFixed(2)}
                  </Text>
                </View>
              </View>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            setFormData({ ...formData, mileageLimit: "unlimited" })
          }
        >
          <View style={styles.radioButton}>
            {formData.mileageLimit === "unlimited" && (
          ...
          <View style={styles.kmInputCard}>
            ...
            <View style={styles.kmInputContainer}>
              <TextInput
                ...
                onChangeText={(text) =>
                  setFormData({ ...formData, dailyKm: text })
                }
                keyboardType="number-pad"
                placeholder="200"
              />
