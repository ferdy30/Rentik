...
export default function RateExperience() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // 0. Guard: prevent duplicate reviews for same reservation
      ...
      if (!existingDocs.empty) {
        Alert.alert(
          "Ya calificaste",
          "Ya enviaste una reseña para este viaje.",
      ...
      // 2. Update Vehicle AND Host rating atomically
      const vehicleRef = doc(db, "vehicles", vehicleId);
      const ownerRef = doc(db, "users", ownerId);

      await runTransaction(db, async (transaction) => {
        const [vehicleDoc, ownerDoc] = await Promise.all([
        ...
        // Update host/owner profile rating
        if (ownerDoc.exists()) {
          const ownerData = ownerDoc.data();
          const currentOwnerRating = ownerData.rating || 0;
          const currentOwnerCount = ownerData.reviewCount || 0;
          const newOwnerCount = currentOwnerCount + 1;
          const newOwnerRating =
            (currentOwnerRating * currentOwnerCount + rating) / newOwnerCount;
          transaction.update(ownerRef, {
            rating: Number(newOwnerRating.toFixed(1)),
            reviewCount: newOwnerCount,
            updatedAt: serverTimestamp(),
          });
        }
      });
      ...
      if (error.code === "permission-denied") {
        errorMessage =
          "No tienes permiso para enviar esta calificación. Por favor contacta con soporte.";
      } else if (error.code === "unavailable") {
        errorMessage =
          "Sin conexión a internet. Verifica tu conexión e intenta de nuevo.";
      } else if (error.code === "not-found") {
        errorMessage =
          "El vehículo no fue encontrado. La calificación no se guardó.";
      }
