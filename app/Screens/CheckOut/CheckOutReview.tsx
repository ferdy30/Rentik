...
export default function CheckOutReview() {
  ...
  const { checkOutId, reservationId } = route.params as {
    checkOutId: string;
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        logger.warn("No Check-in found for this reservation");
      }
    } catch (error: any) {
      logger.error("Error loading checkout data", { error: error.message });
      Alert.alert("Error", "No se pudieron cargar los datos del check-out.");
      navigation.goBack();
    } finally {
      setLoading(false);
    }
  }, [checkOutId, reservationId, navigation]);
  ...
  const handleFinalize = async () => {
    if (!signature) {
      Alert.alert(
        "Firma requerida",
        "Por favor firma para confirmar el estado del vehículo.",
      );
      return;
    }
    ...
    try {
      ...
      // 2. Update Reservation  — mark checkout complete + link checkOut doc
      ...
      logger.error("Error finalizing check-out", { error: error.message });
      Alert.alert(
        "Error",
        "No se pudo finalizar el proceso. Intenta de nuevo.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const distanceDriven =
    (checkOut.conditions?.odometer || 0) - (checkIn.conditions?.odometer || 0);
  const fuelDiff =
    (checkOut.conditions?.fuelLevel || 0) -
    (checkIn.conditions?.fuelLevel || 0);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.summaryRow}>
            <View style={styles.summaryItem}>
              ...
              <Text style={styles.summaryValue}>
                {checkIn.conditions?.fuelLevel}%
              </Text>
            </View>
            <View style={styles.summaryItem}>
              <Text style={styles.summaryLabel}>Combustible Final</Text>
              <Text style={styles.summaryValue}>
                {checkOut.conditions?.fuelLevel}%
              </Text>
            </View>
          </View>
        ...
        {/* Damages */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Reporte de Daños</Text>
              ...
              <View key={index} style={styles.damageItem}>
                ...
                <View style={styles.damageInfo}>
                  <Text style={styles.damageType}>
                    {damage.type.toUpperCase()} - {damage.severity}
                  </Text>
                  <Text style={styles.damageLocation}>{damage.location}</Text>
                  {damage.notes ? (
                    <Text style={styles.damageNotes}>{damage.notes}</Text>
                  ) : null}
                </View>
              </View>
            ))
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.disclaimer}>
            Al firmar, confirmo que he devuelto el vehículo en las condiciones
            descritas anteriormente y acepto los cargos adicionales que puedan
            aplicar por combustible faltante, exceso de kilometraje o daños
            reportados.
          </Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[
            styles.submitButton,
            (!signature || submitting) && styles.buttonDisabled,
          ]}
