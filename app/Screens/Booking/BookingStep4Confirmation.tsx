...
export default function BookingStep4Confirmation() {
  ...
  const calculateTotal = () => {
    ...
    const rentikCommission = subtotal * 0.1;
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!isAvailable) {
        Alert.alert(
          "No disponible",
          "Lo sentimos, estas fechas ya no están disponibles. Por favor selecciona otras.",
        );
        return;
      }
      ...
      if (!arrendadorId) {
        ...
        Alert.alert(
          "Error",
          "No se pudo identificar al propietario del vehículo. Por favor contacta soporte.",
        );
        return;
      }
      ...
      const reservationData: any = {
        ...
        priceBreakdown: {
          ...
          ownerEarnings:
            rentalCost + extrasTotal + deliveryFee - rentalCost * 0.1,
          total,
        },
      };
      ...
      if (deliveryCoords) {
        ...
        reservationData.deliveryCoordinates = deliveryCoords; // Alias for compatibility
        reservationData.pickupCoordinates = deliveryCoords; // Also save as pickupCoordinates for map display
        reservationData.returnCoordinates = deliveryCoords; // Return to same delivery location
        ...
        reservationData.returnCoordinates = vehicle.coordinates; // Return to same pickup location
      }
      if (message) {
      ...
      logger.info("Creating reservation", {
        ...
        isDelivery: reservationData.isDelivery,
        hasDeliveryCoords: !!deliveryCoords,
        hasPickupCoords: !!reservationData.pickupCoordinates,
        hasReturnCoords: !!reservationData.returnCoordinates,
      });
        ...
        [
          {
            ...
            onPress: () =>
              (navigation as any).navigate("HomeArrendatario", {
                screen: "Viajes",
              }),
          },
        ],
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.detailRow}>
            ...
            <View style={styles.detailContent}>
              ...
              <Text style={styles.detailValue}>
                {pTime.toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })}{" "}
                -{" "}
                {rTime.toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </Text>
          ...
          <View style={styles.detailRow}>
            <View style={styles.detailIcon}>
              <Ionicons
                name={isDelivery ? "car-outline" : "location-outline"}
                size={20}
                color="#0B729D"
              />
            </View>
            <View style={styles.detailContent}>
              <Text style={styles.detailLabel}>
                {isDelivery ? "Entrega (Delivery)" : "Ubicación de recogida"}
              </Text>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.sectionSubtitle}>
            Mejora tu experiencia de viaje
          </Text>

          <TouchableOpacity
            style={[
              styles.extraRow,
              extras.insurance && styles.extraRowSelected,
            ]}
            onPress={() =>
              setExtras({ ...extras, insurance: !extras.insurance })
            }
            activeOpacity={0.7}
          >
            <View
              style={[
                styles.extraIcon,
                { backgroundColor: extras.insurance ? "#DBEAFE" : "#F3F4F6" },
              ]}
            >
              <Ionicons
                name="shield-checkmark"
                size={24}
                color={extras.insurance ? "#0B729D" : "#9CA3AF"}
              />
            </View>
            <View style={styles.extraInfo}>
              ...
              <Text style={styles.extraDesc}>
                Cobertura total contra daños y robo
              </Text>
              <Text style={styles.extraPrice}>
                ${EXTRAS_PRICES.insurance}/día
              </Text>
            </View>
            <View
              style={[
                styles.checkbox,
                extras.insurance && styles.checkboxSelected,
              ]}
            >
              {extras.insurance && (
          ...
          <TouchableOpacity
            style={[
              styles.extraRow,
              extras.babySeat && styles.extraRowSelected,
            ]}
          ...
          >
            <View
              style={[
                styles.extraIcon,
                { backgroundColor: extras.babySeat ? "#DBEAFE" : "#F3F4F6" },
              ]}
            >
              <Ionicons
                name="person"
                size={24}
                color={extras.babySeat ? "#0B729D" : "#9CA3AF"}
              />
            </View>
            <View style={styles.extraInfo}>
              ...
              <Text style={styles.extraPrice}>
                ${EXTRAS_PRICES.babySeat} (pago único)
              </Text>
            </View>
            <View
              style={[
                styles.checkbox,
                extras.babySeat && styles.checkboxSelected,
              ]}
            >
              {extras.babySeat && (
                <Ionicons name="checkmark" size={18} color="#fff" />
          ...
          >
            <View
              style={[
                styles.extraIcon,
                { backgroundColor: extras.gps ? "#DBEAFE" : "#F3F4F6" },
              ]}
            >
              <Ionicons
                name="navigate"
                size={24}
                color={extras.gps ? "#0B729D" : "#9CA3AF"}
              />
            </View>
            <View style={styles.extraInfo}>
              <Text style={styles.extraTitle}>GPS Navegador</Text>
              <Text style={styles.extraDesc}>
                Mapas actualizados y tráfico en tiempo real
              </Text>
              <Text style={styles.extraPrice}>${EXTRAS_PRICES.gps}/día</Text>
            </View>
            <View
              style={[styles.checkbox, extras.gps && styles.checkboxSelected]}
            >
              {extras.gps && (
                <Ionicons name="checkmark" size={18} color="#fff" />
              )}
          ...
          {priceDetailsExpanded && (
            <View style={styles.priceBreakdownCard}>
              <View style={styles.priceRow}>
                <View style={styles.priceRowLeft}>
                  <Text style={styles.priceLabel}>
                    ${vehicle.precio} × {days} día{days > 1 ? "s" : ""}
                  </Text>
              ...
              {isDelivery && (
                <View style={styles.priceRow}>
                  <View style={styles.priceRowLeft}>
                    ...
                    <Text style={styles.priceSubtext}>
                      {deliveryDistance
                        ? `${deliveryDistance} - Entrega y recogida`
                        : "Entrega a domicilio"}
                    </Text>
                  </View>
                  <Text style={styles.priceValue}>
              ...
              <View style={styles.priceRow}>
                <View style={styles.priceRowLeft}>
                  <Text style={styles.priceLabel}>
                    Comisión Rentik (20% split)
                  </Text>
                  <Text style={styles.priceSubtext}>
                    10% para cliente, 10% del dueño
                  </Text>
                </View>
                <Text style={styles.priceValue}>
                  ${rentikCommission.toFixed(2)}
                </Text>
              </View>
