...
export default function CheckInConditions() {
  ...
  const handleTakeFuelPhoto = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permiso denegado",
        "Necesitamos acceso a la cámara para verificar el combustible.",
      );
      return;
    }
  ...
  const uploadImage = async (uri: string): Promise<string> => {
    try {
      ...
      // Limpiar archivo temporal en Android
      if (Platform.OS === "android" && uri.startsWith("file://")) {
        try {
          const FileSystem = await import("expo-file-system");
          await FileSystem.deleteAsync(uri, { idempotent: true });
          logger.info("Cleaned fuel photo temp file");
      ...
      logger.error("Error uploading fuel gauge image", {
        error: error.message,
      });
      throw error;
    }
  ...
  const handleContinue = async () => {
    ...
    if (!odometer || parseInt(odometer) <= 0) {
      Alert.alert(
        "Error",
        "Por favor ingresa el kilometraje actual del vehículo.",
      );
      return;
    }
    ...
    if (!fuelGaugePhoto) {
      Alert.alert(
        "Foto requerida",
        "Por favor toma una foto del medidor de combustible.",
      );
      return;
    }
    ...
    try {
      ...
      // Navigate to damage report screen with minimal data
      ...
      logger.error("Error saving check-in conditions", {
        error: error.message,
      });
      Alert.alert(
        "Error",
        "No se pudieron guardar las condiciones. Intenta de nuevo.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <View style={styles.section}>
          ...
          <View style={styles.sliderContainer}>
            ...
            <TouchableOpacity
              ...
              onPress={handleTakeFuelPhoto}
            >
              {fuelGaugePhoto ? (
                <Image
                  source={{ uri: fuelGaugePhoto }}
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <View
              style={[
                styles.badge,
                { backgroundColor: getRatingColor(exteriorCleanliness) + "20" },
              ]}
            >
              <Text
                style={[
                  styles.badgeText,
                  { color: getRatingColor(exteriorCleanliness) },
                ]}
              >
          ...
          <View style={styles.ratingDots}>
            {[1, 2, 3, 4, 5].map((val) => (
              <View
                ...
                style={[
                  styles.ratingDot,
                  val <= exteriorCleanliness && {
                    backgroundColor: getRatingColor(exteriorCleanliness),
                  },
                ]}
              />
            ))}
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <View
              style={[
                styles.badge,
                { backgroundColor: getRatingColor(interiorCleanliness) + "20" },
              ]}
            >
              <Text
                style={[
                  styles.badgeText,
                  { color: getRatingColor(interiorCleanliness) },
                ]}
              >
          ...
          <View style={styles.ratingDots}>
            {[1, 2, 3, 4, 5].map((val) => (
              <View
                ...
                style={[
                  styles.ratingDot,
                  val <= interiorCleanliness && {
                    backgroundColor: getRatingColor(interiorCleanliness),
                  },
                ]}
              />
            ))}
            ...
            <View style={styles.toggleLeft}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.toggleSubtitle}>
                  {smells
                    ? "Se detectan malos olores (cigarrillo, humedad, etc.)"
                    : "Sin olores detectados"}
                </Text>
              </View>
            </View>
            <View style={[styles.toggle, smells && styles.toggleActiveRed]}>
              <View
                style={[styles.toggleThumb, smells && styles.toggleThumbActive]}
              />
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <View
              style={[
                styles.badge,
                { backgroundColor: getRatingColor(tiresCondition) + "20" },
              ]}
            >
              <Text
                style={[
                  styles.badgeText,
                  { color: getRatingColor(tiresCondition) },
                ]}
              >
            ...
            {[1, 2, 3, 4, 5].map((val) => (
              <View
                ...
                style={[
                  styles.ratingDot,
                  val <= tiresCondition && {
                    backgroundColor: getRatingColor(tiresCondition),
                  },
                ]}
              />
            ))}
          ...
          >
            <View style={styles.toggleLeft}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.toggleSubtitle}>
                  Delanteras, traseras e intermitentes
                </Text>
              </View>
            </View>
            <View style={[styles.toggle, lightsWorking && styles.toggleActive]}>
              <View
                style={[
                  styles.toggleThumb,
                  lightsWorking && styles.toggleThumbActive,
                ]}
              />
            </View>
          </TouchableOpacity>
          ...
          >
            <View style={styles.toggleLeft}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.toggleSubtitle}>
                  Tarjeta de circulación y póliza
                </Text>
              </View>
            </View>
            <View
              style={[styles.toggle, documentsPresent && styles.toggleActive]}
            >
              <View
                style={[
                  styles.toggleThumb,
                  documentsPresent && styles.toggleThumbActive,
                ]}
              />
            </View>
          </TouchableOpacity>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity
          style={[
            ...
            (!odometer || !fuelGaugePhoto) && { opacity: 0.6 },
          ]}
