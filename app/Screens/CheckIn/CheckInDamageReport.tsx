...
export default function CheckInDamageReport() {
  ...
  const [damageType, setDamageType] = useState<
    "scratch" | "dent" | "stain" | "crack" | "other"
  >("scratch");
  ...
  useEffect(() => {
    ...
    return () => {
      isMounted = false;
      unsubscribe();
      logger.info("CheckInDamageReport listener cleanup");
    };
  }, [checkInId]);
  ...
  const handleTakeDamagePhoto = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permiso denegado",
        "Necesitamos acceso a la cámara para reportar daños.",
      );
      return;
    }
  ...
  const uploadImage = async (uri: string): Promise<string> => {
    try {
      ...
      logger.error("Error uploading damage image", { error: error.message });
      throw error;
    }
  };
  ...
  const handleContinue = () => {
    navigation.replace("CheckInKeys", {
      reservation: {
        id: reservation.id,
  ...
  const renderDamageItem = ({
    item,
  }: {
    item: CheckInReport["damages"][0];
  }) => (
      ...
      <View style={styles.content}>
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoText}>
            Reporta cualquier daño existente en el vehículo. Si no hay daños
            nuevos, puedes continuar.
          </Text>
        </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtext}>
                El vehículo parece estar en perfectas condiciones
              </Text>
            </View>
          }
        />
            ...
            >
              {[
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    damageLocation === loc && styles.chipSelected,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.chipText,
                      damageLocation === loc && styles.chipTextSelected,
                    ]}
                  >
                    {loc}
                  </Text>
            ...
            >
              {[
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    damageType === type.id && styles.chipSelected,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.chipText,
                      damageType === type.id && styles.chipTextSelected,
                    ]}
                  >
                    {type.label}
                  </Text>
