...
import React, { useCallback, useEffect, useState } from "react"; // Removed unused useMemo
import {
    ActivityIndicator,
    Alert,
    Image,
...
import { subscribeToCheckIn } from "../../services/checkIn";
...
export default function TripDetails() {
  ...
  // Live subscription to the checkIn document (to detect partial-sign state)
  const [liveCheckIn, setLiveCheckIn] = useState<any>(null);
  ...
  const isCheckInCompleted = useCallback(() => {
    ...
    const hasCompletedStatus =
      (reservation.checkIn as any)?.status === "completed";
    const isInProgress = reservation.status === "in-progress";
    // Also check the live checkIn document (both signatures)
    const liveBothSigned = !!(
      liveCheckIn?.signatures?.renter && liveCheckIn?.signatures?.owner
    );
    const liveCompleted = liveCheckIn?.status === "completed";
  ...
  }, [reservation, liveCheckIn]);

  // Subscribe to checkIn document when a check-in is started
  useEffect(() => {
    ...
    const unsub = subscribeToCheckIn(checkInId, (data) => setLiveCheckIn(data));
    return () => unsub();
  }, [reservation?.checkIn?.id]);
    ...
    useCallback(() => {
      ...
      const fetchFreshReservation = async () => {
        try {
          ...
          console.error(
            "[TripDetails] Error fetching fresh reservation:",
            error,
          );
        }
      };
    ...
    const fetchHostInfo = async () => {
      ...
      try {
        const hostDoc = await getDoc(
          doc(db, "users", reservation.arrendadorId),
        );
        if (hostDoc.exists()) {
  ...
  // const timelineSteps = useMemo(() => getTimelineSteps(), [getTimelineSteps]); // No utilizado actualmente

  const handleCancelReservation = async () => {
    Alert.alert(
      "Cancelar solicitud",
      "¿Estás seguro que deseas cancelar esta solicitud de reserva?",
      [
        { text: "No", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                "Solicitud cancelada",
                "Tu solicitud de reserva ha sido cancelada correctamente.",
              );
              navigation.goBack();
            } catch (error) {
  ...
  // Funciones de utilidad que podrían ser útiles en futuras versiones:
  /*
  ...
  */
  ...
  const getStatusSubtitle = (status: string, res: Reservation) => {
    ...
    switch (status) {
      ...
      case "confirmed":
        return daysUntil > 0
          ? `Tu viaje comienza en ${daysUntil} día${daysUntil !== 1 ? "s" : ""}`
          : "Tu viaje está listo para comenzar";
      case "in-progress":
        return "¡Disfruta el camino!";
      case "completed":
        return "¡Gracias por usar Rentik!";
      case "cancelled":
        return res.cancelReason || "La reserva fue cancelada";
      case "denied":
        return res.denialReason || "El anfitrión no pudo aceptar tu reserva";
      default:
        return "";
    }
  };
  ...
  const getStatusStyle = () => {
    if (!reservation)
      return { bg: "#F3F4F6", color: "#374151", icon: "information-circle" };
  ...
  const handleCallEmergency = () => {
    Alert.alert("🚨 Emergencia", "¿Qué tipo de ayuda necesitas?", [
      {
        text: "911 - Emergencia",
  ...
  const handleChat = async () => {
    ...
    if (!reservation.arrendadorId) {
      Alert.alert(
        "Error",
        "No se pudo identificar al anfitrión de esta reserva.",
      );
      return;
    }
    ...
    try {
      ...
      const renterName = renterDoc.exists()
        ? renterDoc.data().nombre || "Usuario"
        : "Usuario";
      const hostName = hostDoc.exists()
        ? hostDoc.data().nombre || "Anfitrión"
        : "Anfitrión";
      ...
      // chatId is used internally by createChatIfNotExists to navigate
      void chatId;
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <View
          style={[styles.statusHeader, { backgroundColor: statusStyle.bg }]}
        >
          <View
            style={[
          ...
          <View style={styles.statusContent}>
            ...
            <Text
              style={[
                ...
                { color: statusStyle.color, opacity: 0.8 },
              ]}
            >
                ...
                <View style={styles.hostDetails}>
                  ...
                  <View style={styles.hostStats}>
                    <View style={styles.hostStatBadge}>
                      ...
                      <Text style={styles.hostStatText}>
                        {(hostInfo.rating || 0).toFixed(1)}
                      </Text>
                    </View>
                    <View style={styles.hostStatBadge}>
                      <Ionicons name="car-sport" size={14} color="#6B7280" />
                      <Text style={styles.hostStatText}>
                        {hostInfo.completedTrips} viajes
                      </Text>
                    </View>
                  </View>
                  <View style={styles.hostVerificationContainer}>
                    <Ionicons
                      ...
                      color="#10B981"
                    />
                    <Text style={styles.hostVerificationText}>
                      Usuario Verificado
                    </Text>
                  </View>
                </View>
                ...
                {hostInfo.telefono && (
                  ...
                  >
                    ...
                    <View style={styles.hostContactTextWrapper}>
                      ...
                      <Text style={styles.hostContactValue}>
                        {hostInfo.telefono}
                      </Text>
                    </View>
                    <Ionicons
                ...
                <View style={styles.activeTripCard}>
                  <View style={styles.activeTripHeader}>
                    ...
                    <View style={styles.activeTripHeaderText}>
                      ...
                      <Text style={styles.activeTripSubtitle}>
                        {daysRemaining > 0
                          ? `${daysRemaining}d ${hoursRemainingMod}h restantes`
                  ...
                  <View style={styles.activeTripActions}>
                    ...
                    >
                      ...
                      <Text style={styles.activeTripActionText}>
                        Navegación
                      </Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      style={[
                        styles.activeTripActionButton,
                        styles.activeTripActionEmergency,
                      ]}
                      onPress={handleCallEmergency}
                    >
          ...
          reservation.status !== "cancelled" && (
            <View style={styles.sectionWithPadding}>
              <TripTimeline
                ...
                isRenter={true}
                checkInCompleted={
                  reservation.checkIn?.completed ||
                  reservation.status === "in-progress" ||
                  reservation.status === "completed"
                }
                checkOutCompleted={
                  reservation.checkOut?.completed ||
                  reservation.status === "completed"
                }
              />
            </View>
            ...
            <View style={styles.priceCardNew}>
              <View style={styles.priceItemRow}>
                ...
                <Text style={styles.priceItemValue}>
                  $
                  {(
                    (reservation.priceBreakdown.pricePerDay || 0) *
                    (reservation.priceBreakdown.days || 0)
                  ).toFixed(2)}
                </Text>
              </View>
              <Text style={styles.priceItemDetail}>
                {reservation.priceBreakdown.days} días × $
                {reservation.priceBreakdown.pricePerDay}/día
              </Text>
              ...
              {reservation.priceBreakdown.extrasTotal > 0 && (
                <>
                  ...
                  <View style={styles.priceItemRow}>
                    ...
                    <Text style={styles.priceItemValue}>
                      $
                      {(reservation.priceBreakdown.extrasTotal || 0).toFixed(2)}
                    </Text>
                  </View>
                </>
              )}
              ...
              {reservation.priceBreakdown.deliveryFee > 0 && (
                <>
                  ...
                  <View style={styles.priceItemRow}>
                    ...
                    <Text style={styles.priceItemValue}>
                      $
                      {(reservation.priceBreakdown.deliveryFee || 0).toFixed(2)}
                    </Text>
                  </View>
                </>
              )}
              ...
              <View style={styles.priceItemRow}>
                ...
                <Text style={styles.priceItemValue}>
                  $
                  {(
                    reservation.priceBreakdown.rentikCommission ??
                    reservation.priceBreakdown.serviceFee ??
                    0
                  ).toFixed(2)}
                </Text>
              </View>
              ...
              <View style={styles.priceTotalRowNew}>
                <View>
                  ...
                  <Text style={styles.pricePaymentMethod}>
                    Pagado con tarjeta
                  </Text>
                </View>
                <Text style={styles.priceTotalValueNew}>
                  $
                  {(
                    reservation.priceBreakdown.total ||
        ...
        <View style={styles.sectionWithPadding}>
          ...
          <View style={styles.infoCardAlert}>
            <View style={styles.infoCardAlertHeader}>
              ...
              <Text style={styles.infoCardAlertTitle}>
                Política de cancelación
              </Text>
            </View>
            <Text style={styles.infoCardAlertText}>
              Cancelación gratuita hasta 48 horas antes del inicio.
              Cancelaciones con menos de 48h tienen una penalización del 20% del
              total.
            </Text>
            {reservation.status === "confirmed" &&
              (() => {
                ...
                return (
                  <View
                    style={[
                      styles.alertBadge,
                      {
                        backgroundColor: canCancelFree ? "#DCFCE7" : "#FEE2E2",
                      },
                    ]}
                  >
                    <Ionicons
            ...
            <View style={styles.keyInfoItem}>
              ...
              <Text style={styles.keyInfoValue}>
                {reservation.priceBreakdown?.days
                  ? `${reservation.priceBreakdown.days * 200} km`
                  : "Ilimitado"}
              </Text>
          ...
          <View style={styles.documentsCardNew}>
            <View style={styles.documentsHeaderNew}>
              ...
              <Text style={styles.documentsCardTitle}>
            ...
            {reservation.pickupCoordinates || reservation.deliveryCoords ? (
              <>
                ...
                >
                  ...
                  <Text style={styles.directionsButtonSecondaryText}>
                    Ver en mapa
                  </Text>
                </TouchableOpacity>
              </>
          ...
          {reservation.status === "denied" && reservation.denialReason && (
            <View
              style={[
                ...
                { backgroundColor: "#FEF9C3", marginTop: 8 },
              ]}
            >
              <Ionicons name="information-circle" size={16} color="#92400E" />
              <Text
                style={[
                  styles.checkInBannerText,
      ...
      {(reservation.status === "confirmed" ||
        ...
        (() => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <Text
                    style={[styles.checkInBannerText, { color: "#166534" }]}
                  >
          ...
          // Ventana de check-in: desde 24h antes hasta 4h después del inicio (gracia)
          const canCheckIn =
            hoursUntilStart <= 24 && hoursUntilStart > -4 && hoursUntilEnd > 0;

          // Un check-in ya iniciado siempre se puede continuar, sin importar la ventana
          const hasActiveCheckIn = !!(
            reservation.checkIn && !isCheckInCompleted()
          );
          // El arrendatario ya firmó pero el dueño aún no
          const renterAlreadySigned = !!liveCheckIn?.signatures?.renter;
          const awaitingOwnerSignature =
            hasActiveCheckIn && renterAlreadySigned;
          const buttonEnabled = canCheckIn || hasActiveCheckIn;

          let buttonText = "Preparar Check-in";
          let statusBanner = null;
          let onPressAction = () =>
            navigation.navigate("CheckInPreparation", {
              reservation,
              isArrendador: false,
            });

          // ✅ FIX: Arrendatario ya firmó, solo espera la firma del dueño
          if (awaitingOwnerSignature) {
            buttonText = "Ver estado del Check-in";
            onPressAction = () =>
              navigation.navigate("CheckInComplete", {
                reservation,
                checkInId: reservation.checkIn?.id,
            ...
            statusBanner = (
              <View
                style={[styles.checkInBanner, { backgroundColor: "#FEF3C7" }]}
              >
                <Ionicons name="hourglass-outline" size={18} color="#D97706" />
                <Text style={[styles.checkInBannerText, { color: "#92400E" }]}>
                  Tu firma está guardada ✅ · Esperando que el anfitrión firme
                </Text>
              </View>
            );
            ...
            statusBanner = (
              ...
              >
                <Ionicons name="hourglass-outline" size={18} color="#D97706" />
                <Text style={[styles.checkInBannerText, { color: "#92400E" }]}>
                  Check-in en proceso · Continúa donde lo dejaste
                </Text>
              </View>
            );
          } else if (!canCheckIn) {
            ...
            if (hoursUntilStart > 24) {
              ...
              // Entre -4h y el fin del viaje sin check-in iniciado
            ...
            statusBanner = (
              <View
                style={[styles.checkInBanner, { backgroundColor: "#DCFCE7" }]}
              >
