...
const PROMOTIONS = [
  {
    ...
    subtitle: "Viaja con libertad",
    image:
      "https://cdn.pixabay.com/photo/2020/03/27/22/06/santa-ana-4975147_1280.jpg",
  },
  {
    id: "2",
    title: "Tu Aventura Espera",
    subtitle: "Descubre nuevos destinos",
    image:
      "https://cdn.pixabay.com/photo/2022/06/13/14/58/road-7260175_1280.jpg",
  },
  {
    id: "3",
    title: "Viaja Seguro",
    subtitle: "Atención 24/7 para ti",
    image:
      "https://media.istockphoto.com/id/1469729479/es/foto/atardecer-en-la-playa.jpg?s=612x612&w=0&k=20&c=sk24uTMckudyN9oeLoNRUTjgWgMi6ymtpFpx6EcTCHA=",
  },
];
...
export default function BuscarScreen() {
  ...
  const requestLocationPermission = useCallback(async () => {
    try {
      ...
      if (status === "granted") {
        ...
        setLocationError(
          "Necesitas activar los permisos de ubicación para ver resultados cercanos.",
        );
      }
    } catch (error) {
      ...
      setLocationError(
        "Ocurrió un problema al solicitar permisos de ubicación.",
      );
    }
  }, []);
  ...
  const getUserLocation = async () => {
    try {
      ...
      setLocationError(
        "No pudimos obtener tu ubicación. Activa los servicios de ubicación para ver resultados cercanos.",
      );
    } finally {
    ...
    async (reset: boolean = false) => {
      try {
        ...
        const mappedVehicles: Vehicle[] = data.map((v) => ({
          ...
          id: v.id!,
          imagen:
            v.photos?.front || v.imagen || "https://via.placeholder.com/400",
          imagenes: v.photos
            ? [
                v.photos.front,
                v.photos.sideLeft,
            ...
            : [v.imagen].filter((img) => img && img.length > 0),
          rating: v.rating || 0,
  ...
  const filteredVehicles = vehicles.filter((vehicle) => {
    ...
    if (selectedCategory !== "all") {
      if (selectedCategory === "airport") {
        ...
        if (
          ...
          !vehicle.ubicacion.toLowerCase().includes("aeropuerto")
        ) {
          return false;
        }
      }
    ...
    // Advanced filters
    if (
      vehicle.precio < advancedFilters.priceRange[0] ||
      vehicle.precio > advancedFilters.priceRange[1]
    ) {
      return false;
    }
    if (
      advancedFilters.vehicleTypes.length > 0 &&
      !advancedFilters.vehicleTypes.includes(vehicle.tipo)
    ) {
      return false;
    }
    if (
      advancedFilters.transmision.length > 0 &&
      !advancedFilters.transmision.includes(vehicle.transmision)
    ) {
      return false;
    }
    if (
      advancedFilters.fuelTypes.length > 0 &&
      !advancedFilters.fuelTypes.includes(vehicle.combustible)
    ) {
      return false;
    }
    if (
      vehicle.anio < advancedFilters.yearRange[0] ||
      vehicle.anio > advancedFilters.yearRange[1]
    ) {
      return false;
    }
    if (advancedFilters.features.length > 0) {
      ...
      const hasAllFeatures = advancedFilters.features.every((feat) =>
        vehicle.caracteristicas.includes(feat),
      );
  ...
  const recommended = vehiclesWithDistance
    .filter((v) => v.badges?.includes("Más rentado") || v.rating >= 4.8)
    .slice(0, 4);
            ...
            <>
              ...
              {[5, 10, 20, 50].map((radius) => (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setSelectedRadius(selectedRadius === radius ? null : radius)
                  }
                >
                  <Ionicons
                    name="location"
            ...
            <View>
              ...
              >
                <Text style={styles.sectionTitle}>
                  {searchQuery || selectedCategory !== "all"
                    ? `Resultados (${vehiclesWithDistance.length})`
        ...
        <View style={styles.mapContainer}>
                  ...
                  <View style={{ padding: 6 }}>
                    ...
                    >
                      ...
                      {vehicle.distanceText && (
                        <Text
                          style={{ fontSize: 10, color: "#fff", marginTop: 2 }}
                        >
                          {vehicle.distanceText}
                        </Text>
                      )}
          ...
          {locationPermission !== "granted" && (
            ...
            >
              ...
              <Text style={{ color: "#92400E", fontSize: 13, flex: 1 }}>
                Activa los permisos de ubicación para ver tu posición en el
                mapa.
              </Text>
              <TouchableOpacity onPress={() => Linking.openSettings()}>
                <Text style={{ color: "#92400E", fontWeight: "700" }}>
            ...
            >
              {[5, 10, 20, 50].map((radius) => (
                <TouchableOpacity
                  ...
                  style={{
                    backgroundColor:
                      selectedRadius === radius ? "#0B729D" : "#fff",
                    paddingHorizontal: 16,
                    paddingVertical: 8,
                    borderRadius: 20,
                    borderWidth: 1,
                    borderColor:
                      selectedRadius === radius ? "#0B729D" : "#E5E7EB",
                    shadowColor: "#000",
                    shadowOffset: { width: 0, height: 2 },
                    shadowOpacity: 0.1,
                    shadowRadius: 3,
          ...
          >
            <Text style={{ fontSize: 13, fontWeight: "600", color: "#032B3C" }}>
              {vehiclesWithDistance.filter((v) => v.coordinates).length}{" "}
              vehículos
            </Text>
          </View>
