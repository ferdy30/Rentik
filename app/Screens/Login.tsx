...
const Login = ({ navigation }: any) => {
  ...
  const googleWebClientId = (Constants.expoConfig?.extra as any)
    ?.googleWebClientId;
  const googleIosClientId = (Constants.expoConfig?.extra as any)
    ?.googleIosClientId;
  const googleAndroidClientId = (Constants.expoConfig?.extra as any)
    ...
    try {
      ...
      if (!googleWebClientId && !googleIosClientId && !googleAndroidClientId) {
        Alert.alert(
          ...
          "Agrega tus Google Client IDs en app.json (expo.extra.*)",
        );
        return;
      }
      ...
      if (res.type !== "success") {
        if (res.type !== "dismiss")
          Alert.alert("Google", "Inicio de sesión cancelado.");
        return;
      }
      const idToken =
        (res.params as any)?.id_token || (res as any)?.authentication?.idToken;
      if (!idToken) {
        Alert.alert(
          "Google",
          "No se recibió idToken. Revisa configuración de OAuth.",
        );
        return;
      }
    ...
    try {
      ...
      try {
        ...
        const routeNames: string[] =
          state?.routeNames || state?.routes?.map((r: any) => r.name) || [];
      ...
      >
        <View style={styles.headerContainer}>
          <View style={styles.logoContainer}>
            <Image
              style={styles.logo}
              source={require("../../assets/images/RentikLogo.png")}
              resizeMode="contain"
            />
          </View>
          ...
          <View style={styles.inputGroup}>
            ...
            >
              <Ionicons
                name="mail-outline"
                size={20}
                color={email.length > 0 ? colors.primary : "#9CA3AF"}
          ...
          <View style={styles.inputGroup}>
            ...
            >
              <Ionicons
                ...
                size={20}
                color={password.length > 0 ? colors.primary : "#9CA3AF"}
                style={styles.inputIcon}
              />
              <TextInput
        ...
        {__DEV__ && (
          <TouchableOpacity style={styles.devButton} onPress={resetOnboarding}>
